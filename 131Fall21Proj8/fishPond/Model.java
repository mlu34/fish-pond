...
public class Model {
	...
	public void moveFish() {
		for(Fish f: fish) {
			if(f.isAlive()) {
				...
				f.move();
				for(Plant p: plants) {
					if(p.isAlive() && p.getRow() == f.getRow() 
							&& p.getCol() == f.getCol()) {
						fishEatPlant(f, p);
					}
				}
				for(int i = 0; i < fish.size(); i++) {
					if(fish.indexOf(f) != i && fish.get(i).getRow() == f.getRow() 
							&& fish.get(i).getCol() == f.getCol()) {
						f.fight(fish.get(i));
					}
				}
	...
	public void addPlant(Plant p) {
		...
		for(Plant indexPlant: plants) {
			if(p.getRow() == indexPlant.getRow() && 
					p.getCol() == indexPlant.getCol()) {
				throw new IllegalPlantPositionException
	...
	public void addFish(Fish f){
		...
		for(Fish indexFish: fish) {
			if(f.getRow() == indexFish.getRow() && 
					f.getCol() == indexFish.getCol()) {
				throw new IllegalFishPositionException
